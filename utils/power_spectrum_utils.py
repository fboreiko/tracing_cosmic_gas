...
class Binner3D:
    ...
    On this pipeline's log grid (681 cMpc/h, 25 bins per decade, 384^3 against
    2048^2) it takes the RMS disagreement between the two lattices below
    k = 1.5 from 0.89% to 0.36%, and at k ~ 1 from -0.16% to -0.01%. The bins
    matter more here than on a linear grid: their width is a fixed fraction of
    k rather than a shrinking one, so the k^2 dk against k dk difference no
    longer dies away at the small scales. What is left is which exact |k|
    values happen to fall in a bin, which no weighting can fix; it alternates
    in sign bin to bin rather than offsetting the curve.
    """
