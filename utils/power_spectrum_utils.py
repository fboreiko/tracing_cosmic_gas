...
def tsc_window_2d(ngrid, box):
    """W(k) = prod_i sinc^3(k_i Delta/2) on the rfft2 layout, Delta = L/ngrid.

    One factor of the TSC assignment window. The fft2-layout twin in
    pmxlib.painting serves azimuthal_mean and is not this shape.
    """
    kx = fftfreq(ngrid, d=box / ngrid) * 2 * np.pi
    ky = rfftfreq(ngrid, d=box / ngrid) * 2 * np.pi
    return (_sinc_half(kx, box, ngrid)[:, None]
            * _sinc_half(ky, box, ngrid)[None, :]) ** 3


def _sinc_half(k, box, ngrid):
    arg = 0.5 * k * (box / ngrid)
    s = np.ones_like(arg)
    nz = arg != 0
...
def tsc_window_3d(ngrid, box):
    ...
    kx = fftfreq(ngrid, d=box / ngrid) * 2 * np.pi
    kz = rfftfreq(ngrid, d=box / ngrid) * 2 * np.pi
    sx, sz = _sinc_half(kx, box, ngrid), _sinc_half(kz, box, ngrid)
